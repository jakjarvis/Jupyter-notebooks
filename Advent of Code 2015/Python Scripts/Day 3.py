...
# create list of lists for complete list of coordinates of houses visited, containing the original house at [0,0]
Visits = [[0,0]]

# create a list of two variables which is the current cooridinate
Coord = [0,0]

# for each character (direction) in the input, update the coordinate variable and append the coordinates of the new house to the visits list
for char in Dir:
    x = char
    if x == "^":
        Coord[0] = Coord[0]+1
...
# # Count the number of unique houses by appending it to a new list of unique houses visited
...
# Same as before but use a counter to switch appends between two 'Coord' variables. Write them to the same 'Visits' list
...
# Search the list for number of unique entries as before
